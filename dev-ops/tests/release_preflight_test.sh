...
if grep -Fq 'INSTALLER_DISABLED_UNTIL_RELEASE_READINESS' "$SCRIPT"; then
  printf 'release preflight must validate the tested local bootstrap, not require a disabled-installer sentinel\n' >&2
  exit 1
fi
