...
func (s *Store) migrate(ctx context.Context) error {
	for _, statement := range []string{
		...
		`CREATE TABLE IF NOT EXISTS agent_usage_sessions (
			...
			sprint_id TEXT REFERENCES sprints(sprint_id) ON DELETE RESTRICT,
			...
			context_used INTEGER CHECK (context_used IS NULL OR context_used >= 0),
		...
		`CREATE TABLE IF NOT EXISTS agent_usage_snapshots (
			...
			usage_id INTEGER NOT NULL REFERENCES agent_usage_sessions(usage_id) ON DELETE CASCADE,
			turn_seq INTEGER NOT NULL CHECK (turn_seq >= 0),
			...
			UNIQUE(usage_id, turn_seq)
		)`,
		"CREATE INDEX IF NOT EXISTS agent_usage_sessions_project_updated_idx ON agent_usage_sessions(project_id, updated_at DESC)",
		"CREATE INDEX IF NOT EXISTS agent_usage_snapshots_usage_turn_idx ON agent_usage_snapshots(usage_id, turn_seq DESC)",
		"CREATE INDEX IF NOT EXISTS agent_usage_snapshots_captured_idx ON agent_usage_snapshots(captured_at DESC)",
