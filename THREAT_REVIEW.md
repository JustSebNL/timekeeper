...
This review covers the local Go HTTP server, SQLite authority, read-only Sprint Pulse attention endpoint, default-on Pulse Guardian delivery loop, low-frequency local health keep-alive, failure-only service kickers, dashboard and its repository-contained Bootstrap `5.3.8` asset, `tk` CLI, VS Code companion, portable export, SQLite backup mode, and installer posture in the current source tree.
...
| Local service liveness | a five-minute loopback `GET /health` keep-alive is optional and does not write SQLite or request a host power hold; `.sh`/`.bat` kickers act only after a failed health check | OS service and platform power/VM policy still own restart and sleep behavior; do not mistake a keep-alive for a hang guarantee |
